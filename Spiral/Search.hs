...
type SearchFunWHT s m a = (Typeable a, Typed a, Floating (Exp a)) => SPL (Exp a) -> S s m (SPL (Exp a))

-- | Run a search
runSearchWHT :: (Typeable a, Typed a, Floating (Exp a), MonadSpiral m)
          => s
          -> (forall a . SearchFunWHT s m a)
          -> SPL (Exp a)
          -> m (SPL (Exp a))
runSearchWHT s f e = runS (searchWHT f e) s

-- | Search rewrites of the given formula with the given rewriting function.
searchWHT :: forall s m a . (Typeable a, Typed a, Floating (Exp a), MonadSpiral m)
       => (forall a . SearchFunWHT s m a)
       -> SPL (Exp a)
       -> S s m (SPL (Exp a))
searchWHT f = go
  where
    go :: forall a . (Typeable a, Typed a, Floating (Exp a))
       => SPL (Exp a)
       -> S s m (SPL (Exp a))
    go e = rewrite e <|> recurse e

    rewrite :: forall a . (Typeable a, Typed a, Floating (Exp a))
            => SPL (Exp a)
            -> S s m (SPL (Exp a))
    rewrite e = do
        e' <- f e
    ...
    recurse :: forall a . (Typeable a, Typed a, Floating (Exp a))
            => SPL (Exp a)
            -> S s m (SPL (Exp a))
    recurse (Kron e1 e2) =
        whenRewritten ((⊗) <$> go e1 <*> go e2) go
