...
runSFKT :: forall m a . MonadFail m => SFKT m a -> m a
runSFKT m =
    unSFKT m failk $ \x _fk -> return x
  where
    failk :: FK (m a)
    failk = Control.Monad.Fail.fail "no answer"
...
instance MonadFail m => MonadFail (SFKT m) where
    fail msg = SFKT $ \_fk _sk -> Control.Monad.Fail.fail msg
